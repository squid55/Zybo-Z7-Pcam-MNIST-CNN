...
│                    │                             │
│  ② max_pooling_pre (448×448 → 28×28)             │
│      16×16 블록에서 가장 밝은 픽셀 선택            │
│                    │                             │
│  ③ rgb_to_cnn (형식 변환)                         │
│      8bit → 10bit, 픽셀클럭 → 데이터클럭           │
│                    │                             │
│  ④ Conv1 (28×28×1 → 28×28×4)                     │
│      3×3 필터 4개, ReLU                           │
│                    │                             │
│  ⑤ MaxPool1 (28×28×4 → 14×14×4)                  │
│      2×2 윈도우에서 최대값                         │
│                    │                             │
│  ⑥ Conv2 (14×14×4 → 14×14×6)                     │
│      3×3 필터 6개, ReLU                           │
│                    │                             │
│  ⑦ MaxPool2 (14×14×6 → 7×7×6)                    │
│                    │                             │
│  ⑧ Conv3 (7×7×6 → 7×7×8)                         │
│      3×3 필터 8개, ReLU                           │
│                    │                             │
│  ⑨ MaxPool3 (7×7×8 → 3×3×8)                      │
│                    │                             │
│  ⑩ Flatten (3×3×8 = 72개 값으로 펼치기)            │
│                    │                             │
│  ⑪ FC Layer (72 → 10, ReLU)                      │
│      72개 입력 × 10개 출력 뉴런                    │
│                    │                             │
│  ⑫ Argmax (10개 중 가장 큰 값 선택)               │
│                    │                             │
│           Prediction = 3                         │
│           Probability = 891 (87%)                │
└─────────────────────────────────────────────────┘
...
소프트웨어 CNN:
  ┌──────────┐     ┌──────────┐     ┌──────────┐
  │ 전체 이미지 │ ──→ │ 전체 Conv  │ ──→ │ 전체 Pool  │
  │ 메모리 저장 │     │ 한 번에    │     │ 한 번에    │
  └──────────┘     └──────────┘     └──────────┘

하드웨어 CNN (이 프로젝트):
  픽셀 하나 ──→ [Conv 파이프라인] ──→ [Pool 파이프라인] ──→ ...
  다음 픽셀 ──→ [Conv 파이프라인] ──→ [Pool 파이프라인] ──→ ...
  다음 픽셀 ──→ [Conv 파이프라인] ──→ [Pool 파이프라인] ──→ ...
    (카메라에서 나오는 속도 그대로, 별도 메모리 버퍼 불필요)
```
...
```
카메라 출력:  rgb_stream  (8bit R,G,B + Column + Row + New_Pixel)
    │
    ▼ Grayscale 변환 + MaxPool 다운스케일
    │
CNN 입력:    rgb_stream  (8bit Gray만 사용, 28×28)
    │
    ▼ rgb_to_cnn 변환
    │
CNN 내부:    CNN_Stream_T  (10bit Data + Column + Row + Filter + Data_CLK)
    │
    ▼ Conv → Pool → Conv → Pool → Conv → Pool → FC → Argmax
    │
CNN 출력:    Prediction (0~9 정수) + Probability (0~1023 정수)
```
...
```
Step 1: 가로 방향 max (열 16개를 1개로)
  [p0, p1, p2, ..., p15] → max(p0~p15) → col_max

Step 2: 세로 방향 max (행 16개를 1개로)
  Row 0:  col_max → RAM에 저장
  Row 1:  col_max vs RAM → 더 큰 값 저장
  ...
  Row 15: col_max vs RAM → 최종 max → 출력!
```
...
```
문제: 컨볼루션 한 번에 3×3×입력채널 = 여러 번의 곱셈이 필요
     하지만 DSP가 1~2개밖에 없으면?
     → 시간을 늘려서 여러 클럭에 걸쳐 순차 계산!

입력:  [A][B][C][D]________________     (픽셀이 빠르게 연속 도착)
출력:  [A]____[B]____[C]____[D]____     (사이에 빈 시간 삽입)
              ↑ 이 시간 동안 컨볼루션 계산!
```
...
```
입력 픽셀 ──→ [RAM에 한 행 저장] ──→ [타이머로 간격 조절] ──→ 출력
              (falling edge 쓰기)     (Delay_Cnt 카운터)    (rising edge 읽기)
```
...
카메라에서 흘러오는 픽셀:
  Row 0: [a b c d e f ...]
  Row 1: [g h i j k l ...]    ← 순환 버퍼에 저장
  Row 2: [m n o p q r ...]    ← 현재 쓰는 중

3×3 윈도우 출력 (위치 (1,1) 기준):
  [a b c]
  [g h i]  → 이 9개를 순서대로 출력
  [m n o]
```
...
입력 스트림
    │
    ▼
[Row Expander] ── 시간 확장 (계산 시간 확보)
    │
    ▼
[Row Buffer] ── 3×3 윈도우 데이터 생성
    │
    ▼
[ROM] ── 가중치 읽기 (합성 시 Block RAM으로 매핑)
    │
    ▼
[MAC 엔진] ── sum += input × weight  (핵심 연산!)
    │
    ▼
[SUM RAM] ── 여러 사이클에 걸친 부분합 저장
    │
    ▼
[+ Bias → ReLU] ── 바이어스 더하고 음수 제거
    │
    ▼
[OUT RAM] ── 활성화 결과 저장
    │
...
```
문제: Conv1은 4개 필터가 필요 → 곱셈기 4개 필요?
해결: Calc_Cycles = 4로 설정 → 1개 곱셈기를 4번 사용!

클럭 1: 필터 0의 곱셈
클럭 2: 필터 1의 곱셈   ← 같은 DSP 재사용
클럭 3: 필터 2의 곱셈
클럭 4: 필터 3의 곱셈

결과: DSP 4개 대신 1개만 사용! (4배 절약, 대신 4배 느림)
```
...
```
차이점:
  Convolution: sum += input × weight  (곱셈-누적, DSP 필요)
  Pooling:     max = max(current, previous)  (비교만, DSP 불필요!)
```
...
```
Convolution (지역 연결):
  각 출력 뉴런이 3×3 = 9개 입력만 봄
  → Row Buffer 필요 (3행 저장)

Fully Connected (전체 연결):
  각 출력 뉴런이 72개 입력 전부를 봄
  → Row Buffer 불필요! 입력이 순서대로 도착하면 됨
```

**이 프로젝트의 FC 레이어**:

```
입력: 72개 (3×3×8을 1줄로 펼친 것)
출력: 10개 (숫자 0~9 각각의 점수)
가중치: 10 × 73 = 730개 (72 입력 + 1 바이어스)

연산:
  출력[0] = ReLU(w[0,0]×in[0] + w[0,1]×in[1] + ... + w[0,71]×in[71] + bias[0])
  출력[1] = ReLU(w[1,0]×in[0] + w[1,1]×in[1] + ... + w[1,71]×in[71] + bias[1])
  ...
  출력[9] = ReLU(w[9,0]×in[0] + w[9,1]×in[1] + ... + w[9,71]×in[71] + bias[9])
```

**시분할 처리**: `Out_Cycles = 10`이므로, 10개 출력을 1개씩 순차 계산
→ 곱셈기 1개로 10개 뉴런 처리 가능
...
```vhdl
-- 3×3×8 = 72개 값을 1D 인덱스로 매핑
iCycle_1N <= (Row * 3 + Column) * 8 + Filter;

-- 예: Row=1, Column=2, Filter=3
-- → (1×3 + 2)×8 + 3 = 43번째 입력
```
...
```
CNN 결과 → AXI-Lite 레지스터 (ARM CPU에서 읽기)
...
```
부동소수점: 0.75 = 0.75  (float, 32bit)
고정소수점: 0.75 ≈ 96/128  (정수 96, 스케일 128)

이 프로젝트:
  가중치 스케일: 2^7 = 128 → 가중치 128은 실수 1.0을 의미
  값 스케일: 레이어마다 다름 (Out_Offset으로 조절)
```

### Offset 파라미터의 의미

```
Layer 1: Out_Offset = 3 → 출력을 2^3 = 8로 나눔
Layer 2: Out_Offset = 3 → 출력을 2^3 = 8로 나눔
Layer 3: Out_Offset = 5 → 출력을 2^5 = 32로 나눔
FC:      Out_Offset = 6 → 출력을 2^6 = 64로 나눔
```
...
```
입력: 500 (10bit, 스케일 2^3=8 기준 → 실수 62.5)
가중치: 64 (8bit, 스케일 2^7=128 기준 → 실수 0.5)
Offset: 1

연산:
  500 × 64 = 32,000
  + 반올림 바이어스 = 32,000 + 32 = 32,032
  >> (8-1-1) = >> 6 = 32,032 / 64 = 500

결과: 500 (실수로 62.5 × 0.5 = 31.25, 스케일 적용하면 맞음)
```
...
작게 (시분할 처리 — 이 프로젝트 방식):
  Conv1의 4개 필터를 순차 계산 (Calc_Cycles=4)
  → DSP 1개로 충분, 4클럭에 완료
  → 면적 작지만 느림
```
...
```
Slice LUTs:  1,832 / 53,200  =  3.44%  ← 96.5% 남음!
